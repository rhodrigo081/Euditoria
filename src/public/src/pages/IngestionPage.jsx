...
import { validateCpf, validateEmail, validateRequired } from '../utils/validators';

export default function IngestionPage({ setTab, selectedTenant, onBatchCreated }) {
  ...
  const isFormValid = () => {
    ...
    if (!validateRequired(telefoneContato) || telefoneContato.length < 14) return false;
  ...
  const handleFileChange = (e) => {
    ...
    if (file) {
      if (!file.name.toLowerCase().endsWith('.xml')) {
        setErrorMessage('Formato inválido: apenas arquivos com extensão .xml são aceitos.');
        return;
      }
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    if (!isFormValid()) {
      setErrorMessage('Preencha todos os campos obrigatórios com informações válidas antes de submeter.');
      return;
    }
    ...
    try {
      ...
      if (activeMode === 'upload') {
        ...
        res = await api.uploadRawXml('lote_digitado.xml', rawXml, selectedTenant);
      }
      ...
      <div class="bg-slate-900 border border-slate-800 rounded-2xl p-6 shadow-xl">
        <div class="border-b border-slate-800 pb-4 mb-6">
          ...
          <p class="text-xs text-slate-400 mt-1">
            Envie o arquivo XML do eSocial para validação de esquemas XSD, cruzamento da linha do tempo e apuração fiscal em fila assíncrona.
          </p>
        </div>
        ...
        {successResult && (
          <div class="mb-6 p-5 rounded-xl bg-emerald-950/50 border border-emerald-800/80 text-emerald-300 text-xs space-y-2">
            ...
            <p class="text-slate-300">
              Status atual: <span class="font-mono font-bold text-white">{successResult.status}</span> ({successResult.estimatedEvents} eventos estimados).
            </p>
        ...
        <form onSubmit={handleSubmit} class="space-y-6">
          ...
          <div class="bg-slate-950/60 border border-slate-800 p-5 rounded-xl space-y-4">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
              <CharacterCountInput
                ...
                helperText="Limite de 60 caracteres (Prevenção de sobrecarga)"
              />
              ...
              <div class="flex flex-col gap-1.5 w-full">
                ...
                />
                <span class="text-xs text-slate-400">Validação RFC 5322 acionada pelo browser</span>
              </div>
            ...
            <CharacterCountInput
              ...
              helperText="Limite estrito de 200 caracteres para evitar payloads excessivos"
            />
          </div>
          ...
          {activeMode === 'upload' ? (
            <div class="border-2 border-dashed border-slate-700 hover:border-sky-500/60 rounded-xl p-8 text-center bg-slate-950/40 transition">
              ...
              <p class="text-sm font-semibold text-slate-200">
                {selectedFile ? selectedFile.name : 'Arraste o arquivo XML ou clique para selecionar'}
              </p>
            ...
            <div class="space-y-2">
              <div class="flex justify-between items-center">
                <span class="text-xs text-slate-400 font-medium">Cole o envelope XML do lote eSocial abaixo:</span>
                <button
                  type="button"
          ...
          <div class="flex items-center justify-between pt-4 border-t border-slate-800">
            <div class="text-xs text-slate-400 flex items-center gap-1.5">
              ...
              <span>O botão só é habilitado com todos os dados íntegros e validados.</span>
            </div>
